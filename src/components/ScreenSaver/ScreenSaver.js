...
function ScreenSaver({ color = 'white' }) {
  ...
  const [isColorSupported, setIsColorSupported] = React.useState(false);
  ...
  React.useEffect(() => {
    setIsColorSupported(CSS.supports('fill', color))
  }, []);
  ...
  return (
    ...
    >
      {showScreensaver && <path
        ...
        fill={isColorSupported ? color: 'white'}
      ></path>}
    </svg>
  );
}
